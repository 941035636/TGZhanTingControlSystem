...
namespace UMP
{
    public class MediaPlayerStandalone : IPlayer, IPlayerAudio, IPlayerSpu
    {
        ...
        public MediaPlayerStandalone(MonoBehaviour monoObject, GameObject[] videoOutputObjects, PlayerOptionsStandalone options)
        {
            ...
            {
                Log.Debug("Don't support video playback on current platform or you use incorrect UMP libraries!");
                throw new Exception();
            }
        ...
        private string GetAudioDevice(string description)
        {
            ...
            {
                 Log.Debug("GetAudioDevice: Can't get audio output device list for " + audioDevice.Name);
            ...
            try
            {
                while (outputDevice.NextDevice != IntPtr.Zero)
                {
                    ...
                    {
                         Log.Debug("GetAudioDevice: New audio output device \n" +
            ...
            if (string.IsNullOrEmpty(audioDeviceName))
                 Log.Debug("GetAudioDevice: Can't find audio output device - switched to Default");
