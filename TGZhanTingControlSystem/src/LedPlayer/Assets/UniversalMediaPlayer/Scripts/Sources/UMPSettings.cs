        ...
        public static UMPSettings Instance
        {
            get
            {
                if (_instance == null)
                {
                    ...
                    if (_instance == null)
                        Log.Debug(string.Format("[UMPSetting] Could not find settings file '{0}' in UMP 'Resources' folder. " +
                    ...
                    if (Application.isEditor)
                    {
                        if (!_instance.IsValidAssetPath)
                            Log.Debug("[UMPSetting] Asset path is not correct, please check the settings file in UMP 'Resources' folder.");
                    }

                    if ((RuntimePlatform & Desktop) == RuntimePlatform)
                    {
                        if (!ContainsLibVLC(_instance.LibrariesPath))
                            Log.Debug("[UMPSetting] Can't find LibVLC libraries, try to check the settings file in UMP 'Resources' folder.");
                    }
                }
